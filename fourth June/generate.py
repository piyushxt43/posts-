#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate Fourth June carousel images."""
...
def save_image(result: dict, path: Path) -> None:
    item = result["data"][0]
    if item.get("url"):
        req = urllib.request.Request(item["url"], headers={"User-Agent": "fourth-june/1.0"})
        with urllib.request.urlopen(req, timeout=TIMEOUT) as r:
...
def main() -> int:
    ...
    parser.add_argument("ids", nargs="*", help="Carousel id 01 (default: all)")
    ...
    print(f"Launching {len(jobs)} image API calls ({workers} workers)...", flush=True)
