...
def _bg(canvas, doc):
    ...
    canvas.rect(0, letter[1] - 16, letter[0] * 0.32, 6, fill=1, stroke=0)
    # Side rule
    canvas.setStrokeColor(GREY)
    canvas.setLineWidth(0.4)
    canvas.line(MARGIN - 8, MARGIN, MARGIN - 8, letter[1] - 0.65 * inch)
    # Footer
    canvas.setFillColor(GREY)
    canvas.setFont("Helvetica", 7)
    canvas.drawString(MARGIN, 0.38 * inch, "@piyush.glitch  |  5th June Operator Guide  |  June 2026")
    canvas.drawRightString(letter[0] - MARGIN, 0.38 * inch, f"Page {doc.page} of 10")
    canvas.restoreState()
