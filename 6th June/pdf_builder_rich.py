...
def _bg(canvas, doc):
    ...
    canvas.drawString(MARGIN, 0.4 * inch, "@piyush.glitch  |  6th June Claude Operator Guide")
...
def _table(headers, rows, st, col_widths=None):
    hdr = [Paragraph(_cell(h), st["th"]) for h in headers]
    data = [hdr] + [[Paragraph(_cell(c), st["td"]) for c in r] for r in rows]
    if not col_widths:
...
def render_blocks(blocks, st) -> list:
    ...
    for b in blocks:
        ...
        if t == "heading":
            ...
            for item in b["items"]:
                flow.append(Paragraph(_cell(item), st["bullet"], bulletText="-"))
        elif t == "code":
