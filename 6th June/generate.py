#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate all Third June carousel images - ALL API calls in one parallel batch."""
...
def save_image(result: dict, path: Path) -> None:
    item = result["data"][0]
    if item.get("url"):
        req = urllib.request.Request(item["url"], headers={"User-Agent": "sixth-june/1.0"})
        with urllib.request.urlopen(req, timeout=TIMEOUT) as r:
...
def collect_jobs(specs: list, force: bool) -> list[tuple]:
    jobs = []
    for spec in specs:
...
def main() -> int:
    ...
    parser.add_argument("ids", nargs="*", help="Carousel ids 01-12 (default: all)")
    ...
    print(f"Launching {len(jobs)} image API calls at once ({workers} workers)...", flush=True)

    ok = fail = 0
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futures = {
            pool.submit(run_job, api_key, slug, folder, fn, pr): (slug, fn)
            for slug, folder, fn, pr in jobs
        }
        for fut in as_completed(futures):
            ...
            try:
                s, f = fut.result()
                ok += 1
                print(f"OK {s}/{f}", flush=True)
            except Exception as e:
                fail += 1
                print(f"FAIL {slug}/{fn}: {e}", flush=True)

    print(f"\nDone: {ok} ok, {fail} failed out of {len(jobs)}.", flush=True)
