#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate Draftly Growth carousel images - 5 x 8 slides."""
...
def save_image(result: dict, path: Path) -> None:
    item = result["data"][0]
    if item.get("url"):
        req = urllib.request.Request(item["url"], headers={"User-Agent": "draftly-growth/1.0"})
        with urllib.request.urlopen(req, timeout=TIMEOUT) as r:
...
def collect_jobs(specs: list, force: bool) -> list:
    ...
    for spec in specs:
        ...
        (folder / "caption.txt").write_text(spec["caption"], encoding="utf-8")
...
def main() -> int:
    ...
    if not jobs:
        print("All slides exist. Use --force to regenerate.", flush=True)
        return 0
    print(f"Generating {len(jobs)} slides, {args.workers} workers", flush=True)
    ok, fail = 0, 0
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        futs = {
            pool.submit(run_job, api_key, slug, folder, fn, prompt): (slug, fn)
            for slug, folder, fn, prompt in jobs
        }
