#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate 7th June Gemini Pro carousel images."""
...
def save_image(result: dict, path: Path) -> None:
    item = result["data"][0]
    if item.get("url"):
        req = urllib.request.Request(item["url"], headers={"User-Agent": "seventh-june/1.0"})
        with urllib.request.urlopen(req, timeout=TIMEOUT) as r:
...
def main() -> int:
    ...
    parser.add_argument("ids", nargs="*", help="01-02")
